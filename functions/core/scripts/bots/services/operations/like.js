 ...
 * */
const likePost = async (botUser, postId) => {
    // Fetch the post
    ...
    await Promise.all([
        ...
        reactionServices.likePostDB(botUser.uid, postId),

        /// Send Notification to the user and add notification to the firestore
         notifications.sendLikeNotificationToUser(botUser.uid, authorId,metadata)
    ]);
