...
class SoundRecordNotifier extends ChangeNotifier {
  ...
  Future<String> getFilePath() async {
    ...
    if (Platform.isIOS) {
      ...
      String appDirectoryPath = await getAppDirectoryPath();
      Directory newRecordSoundDirectory = Directory('$appDirectoryPath/new_record_sound');
      _sdPath = initialStorePathRecord.isEmpty ? newRecordSoundDirectory.path : initialStorePathRecord;
      print(_sdPath);
    }
  ...
  updateScrollValue(Offset currentValue, BuildContext context) async {
    if (buttonPressed == true) {
      ...
      try {
        ...
        if (position.dx <= MediaQuery.sizeOf(context).width * 0.76) {
          resetEdgePadding();
        } else if (x.dx >= MediaQuery.sizeOf(context).width) {
          edge = 0;
        } else {
          if (x.dx <= MediaQuery.sizeOf(context).width * 0.5) {}
          if (last < x.dx) {
            edge = edge -= x.dx / 200;
            if (edge < 0) {
              edge = 0;
  ...
  Future<bool> audioPermissionCheck({required BuildContext context}) async {
    final audioPermissionStatus = await Permission.microphone.request();

    if (audioPermissionStatus.isPermanentlyDenied || audioPermissionStatus.isRestricted || audioPermissionStatus.isDenied) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        /// reset to initial state
        resetEdgePadding();
        _showAudioPermissionSnackBar(context: context);
      });
    }

    return audioPermissionStatus.isGranted;
  }

  void _showAudioPermissionSnackBar({required BuildContext context}) {
    GayaSnackBar.show(type: GayaSnackBarType.problem, text: GayaStrings.voice_permission_allow.tr, context: context);
  ...
  record({required BuildContext context}) async {
    buttonPressed = true;
    _isAcceptedPermission = await audioPermissionCheck(context: context);
