...
class FlutterFeedReaction extends StatefulWidget {
  ...
  // reactions count on each reaction selection from dialog(Popup)
  final List<int>? reactionsCountList;
...
class _FlutterFeedReactionState extends State<FlutterFeedReaction> with TickerProviderStateMixin {
              ...
              portal: Padding(
                ...
                child: SizedBox(
                  child: Stack(
                    ...
                    children: <Widget>[
                      // Box
                      if (widget.reactions.isNotEmpty) _backgroundBoxBuilder(),
  ...
  void _initAnimationBtnLike() {
    // long press
    animControlBtnLongPress = AnimationController(vsync: this, duration: Duration(milliseconds: durationAnimationBtnLongPress));
    ...
    // short press
    animControlBtnShortPress = AnimationController(vsync: this, duration: Duration(milliseconds: durationAnimationBtnShortPress));
  ...
  void _initAnimationIconWhenDrag() {
    animControlIconWhenDrag = AnimationController(vsync: this, duration: Duration(milliseconds: durationAnimationIconWhenDrag));
  ...
  void _initAnimationIconWhenDragOutside() {
    animControlIconWhenDragOutside = AnimationController(vsync: this, duration: Duration(milliseconds: durationAnimationIconWhenDrag));
  ...
  void _initAnimationBoxWhenDragOutside() {
    animControlBoxWhenDragOutside = AnimationController(vsync: this, duration: Duration(milliseconds: durationAnimationIconWhenDrag));
  ...
  void _initAnimationIconWhenDragInside() {
    animControlIconWhenDragInside = AnimationController(vsync: this, duration: Duration(milliseconds: durationAnimationIconWhenDrag));
  ...
  void _initAnimationIconWhenRelease() {
    animControlIconWhenRelease = AnimationController(vsync: this, duration: Duration(milliseconds: durationAnimationIconWhenRelease));

    zoomIconWhenRelease =
        Tween(begin: 1.8, end: 0.0).animate(CurvedAnimation(parent: animControlIconWhenRelease, curve: Curves.decelerate));

    moveUpIconWhenRelease =
        Tween(begin: 180.0, end: 0.0).animate(CurvedAnimation(parent: animControlIconWhenRelease, curve: Curves.decelerate));

    moveLeftIconLikeWhenRelease =
        Tween(begin: 20.0, end: 10.0).animate(CurvedAnimation(parent: animControlIconWhenRelease, curve: Curves.decelerate));
    moveLeftIconLoveWhenRelease =
        Tween(begin: 68.0, end: 10.0).animate(CurvedAnimation(parent: animControlIconWhenRelease, curve: Curves.decelerate));
    moveLeftIconHahaWhenRelease =
        Tween(begin: 116.0, end: 10.0).animate(CurvedAnimation(parent: animControlIconWhenRelease, curve: Curves.decelerate));
    moveLeftIconWowWhenRelease =
        Tween(begin: 164.0, end: 10.0).animate(CurvedAnimation(parent: animControlIconWhenRelease, curve: Curves.decelerate));
    moveLeftIconSadWhenRelease =
        Tween(begin: 212.0, end: 10.0).animate(CurvedAnimation(parent: animControlIconWhenRelease, curve: Curves.decelerate));
    moveLeftIconAngryWhenRelease =
        Tween(begin: 260.0, end: 10.0).animate(CurvedAnimation(parent: animControlIconWhenRelease, curve: Curves.decelerate));
    ...
    return Opacity(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          border: Border.all(color: const Color(0xff7A24FF), width: 1.5),
        ),
            ...
            Transform.scale(
              scale: isDragging
                  ...
                  : isDraggingOutside
                      ...
                      : zoomIconLike.value,
              child: Container(
                margin: EdgeInsets.only(bottom: 3.r, right: 8.r),
                height: currentIconFocus == i ? 70.0.r : 40.0.r,
                alignment: Alignment.center,
                child: Column(
                  mainAxisAlignment: currentIconFocus == i ? MainAxisAlignment.center : MainAxisAlignment.end,
                  children: <Widget>[
                    (widget.reactionsCountList != null && widget.reactionsCountList?[i] != 0)
                        ? Container(
                            decoration: BoxDecoration(borderRadius: BorderRadius.circular(20.0), color: const Color(0xFF39005C)),
                            padding: const EdgeInsets.only(left: 6.0, right: 6.0, top: 2.0, bottom: 2.0).r,
                            margin: const EdgeInsets.only(bottom: 2.0).r,
                            child: Text(
                              // '${widget.reactions[i].name}',
                              '${widget.reactionsCountList?[i]}',
                              style: TextStyle(fontSize: 7.0.sp, color: Colors.white, fontWeight: FontWeight.w600),
                            ),
                          )
    ...
    return GestureDetector(
      ...
      child: Center(
        child: Row(
          children: [
            Transform.scale(
              scale: !isLongPress ? _handleOutputRangeZoomInIconLike(zoomIconLikeInBtn2.value) : zoomIconLikeInBtn.value,
              child: Transform.rotate(
                angle: !isLongPress ? _handleOutputRangeTiltIconLike(tiltIconLikeInBtn2.value) : tiltIconLikeInBtn.value,
                child: widget.prefix,
              ),
            ),
            if (widget.suffix != null) const SizedBox(width: 5.0),
  ...
  void _onHorizontalDragUpdateBoxIcon(DragUpdateDetails dragUpdateDetail) {
    ...
    if (dragUpdateDetail.localPosition.dy >= -100 && dragUpdateDetail.localPosition.dy <= 100) {
      isDragging = true;
      isDraggingOutside = false;
      ...
      for (int i = 1; i <= widget.reactions.length; i++) {
        if (dragUpdateDetail.globalPosition.dx >= _a && dragUpdateDetail.globalPosition.dx < _b) {
  ...
  void _onTapUpBtn(TapUpDetails? tapUpDetail) {
    if (isLongPress) {
      if (widget.reactions.isNotEmpty) widget.onReactionSelected(widget.reactions[whichIconUserChoose]);
      currentIconFocus = -1;
    }
