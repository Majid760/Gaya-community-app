...
class CubeDialog extends CubeEntity {
  ...
  CubeDialog(this.type,
      {this.dialogId, this.name, this.description, this.occupantsIds, this.photo, this.pinnedMessagesIds, this.adminsIds, this.customData});
  ...
  CubeDialog.fromJson(Map<String, dynamic> json) : super.fromJson(json) {
    ...
    if (pinnedMessagesIdsRaw != null) {
      pinnedMessagesIds = List.of(pinnedMessagesIdsRaw).map((id) => id.toString()).toList();
    }
  ...
  Future<CubeMessage> sendMessage(CubeMessage message) async {
  ...
  Future<AbstractChat> checkInit() {
    ...
    if (CubeDialogType.PRIVATE == type) {
      ...
      if (recipientId == -1) {
        completer.completeError(IllegalStateException("Check you set participant and login to the chat"));
        return completer.future;
      }

      PrivateChatManager? privateChatManager = CubeChatConnection.instance.privateChatManager;
      if (privateChatManager == null) {
        completer.completeError(IllegalStateException("Need login to the chat before perform chat related operations"));
        return completer.future;
      }
      ...
      if (isEmpty(dialogId)) {
        completer.completeError(IllegalStateException("'dialog_id' can't be empty or null for this dialog type"));
        return completer.future;
      }

      GroupChatManager? groupChatManager = CubeChatConnection.instance.groupChatManager;
  ...
  Future<GroupChat> _joinInternal(GroupChat groupChat) {
    ...
    if (CubeDialogType.PRIVATE == type) {
      completer.completeError(IllegalStateException('Unavailable operation for \'PRIVATE\' chat'));
    } else {
...
class CubeDialogCustomData extends CubeBaseCustomObject {
  ...
  CubeDialogCustomData.fromJson(Map<String, dynamic> json) : super.fromJson(json);
