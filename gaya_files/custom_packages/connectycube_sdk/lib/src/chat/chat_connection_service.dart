...
class CubeChatConnection {
  ...
  StreamController<CubeChatConnectionState> _connectionStateStreamController = StreamController.broadcast();
  ...
  Future<CubeUser> login(CubeUser cubeUser, {String? resourceId}) async {
    ...
    if (_chatConnectionState == CubeChatConnectionState.Ready) {
      ...
      _loginChatCompleter?.completeError(IllegalStateException('Already logged in.'));
      return _loginChatCompleter!.future.whenComplete(() {
        _loginChatCompleter = null;
      });
    }

    String resource = isEmpty(resourceId) ? CubeSettings.instance.chatDefaultResource : resourceId!;
    String userJid = getJidForUser(cubeUser.id ?? 0, resource);

    xmpp.Jid jid = xmpp.Jid.fromFullJid(userJid);
    xmpp.XmppAccountSettings account = xmpp.XmppAccountSettings(
        userJid, jid.local, jid.domain, cubeUser.password ?? '', kIsWeb ? 5291 : 5222,
        resource: resource, wsProtocols: ['xmpp']);
    account.reconnectionTimeout = CubeChatConnectionSettings.instance.reconnectionTimeout;
    account.totalReconnections = CubeChatConnectionSettings.instance.totalReconnections;
    ...
    xmpp.Log.logXmpp = false ?? CubeSettings.instance.isDebugEnabled;

    _chatLoginStateSubscription = _connection!.connectionStateStream.listen((state) {
...
//    });
  ...
  void _onConnectionStateChangedInternal(xmpp.XmppConnectionState state, CubeUser cubeUser) {
    switch (state) {
      ...
      case xmpp.XmppConnectionState.StartTlsFailed:
        ...
        _notifyChatLoginError(ChatConnectionException(message: "Open connection error: StartTlsFailed"));

        break;
      case xmpp.XmppConnectionState.AuthenticationNotSupported:
        log("Chat connection AuthenticationNotSupported", TAG);
        _setConnectionState(CubeChatConnectionState.AuthenticationFailure);
        _notifyChatLoginError(ChatConnectionException(message: "Open connection error: AuthenticationNotSupported"));
      ...
      case xmpp.XmppConnectionState.AuthenticationFailure:
        ...
        _notifyChatLoginError(ChatConnectionException(message: "Open connection error: AuthenticationFailure"));
      ...
      case xmpp.XmppConnectionState.ForcefullyClosed:
        ...
        _notifyChatLoginError(ChatConnectionException(message: "Open connection error: ForcefullyClosed"));
  ...
  void relogin() {
    if (currentUser == null) {
      throw IllegalStateException("Call 'login(cubeUser)' first before use 'relogin()'");
    }
  ...
  Future<int> getLasUserActivity(int userId) {
    if (lastActivityManager == null) {
      return Future.error(ChatConnectionException(message: CHAT_NOT_CONNECTED_ERROR));
    }

    return lastActivityManager!.getLastActivity(userId);
  }

  Future<void> subscribeToUserLastActivityStatus(int userId, {Function(int)? callback}) {
    if (lastActivityManager == null) {
      return Future.error(ChatConnectionException(message: CHAT_NOT_CONNECTED_ERROR));
    }

    return lastActivityManager!.subscribeToUserLastActivityStatus(userId, callback: callback);
  }
  ...
  Stream<LastActivitySubscriptionEvent>? get lastActivityStream => lastActivityManager?.lastActivityStream ?? null;
  ...
  Future<void> _enableMobileOptimisation(bool enable) {
    ...
    CubeChatConnection.instance.asyncStanzaSender?.sendAsync(enableMobileOptimisation, (resultStanza, exception) {
  ...
  MessagesStatusesManager? get messagesStatusesManager {
    if (isAuthenticated()) {
      _messagesStatusesManager = MessagesStatusesManager.getInstance(_connection!);
    }
  ...
  MessagesReactionsManager? get messagesReactionsManager {
    if (isAuthenticated()) {
      _messagesReactionsManager = MessagesReactionsManager.getInstance(_connection!);
    }
...
class CubeChatConnectionSettings {
  static final CubeChatConnectionSettings _singleton = CubeChatConnectionSettings._internal();
