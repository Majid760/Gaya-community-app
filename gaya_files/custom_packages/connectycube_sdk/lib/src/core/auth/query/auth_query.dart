...
abstract class BaseAuthQuery<T> extends AutoManagedQuery<T> {
  ...
  setBody(RestRequest request) {
    ...
    if (_provider != null) {
      if (_provider == CubeProvider.FIREBASE_PHONE) {
        _convertFirebaseParamsToJson(parameters);
      } else {
  ...
  _convertFirebaseParamsToJson(Map<String, dynamic> parameters) {
    Map firebaseParamsJson = Map();

    firebaseParamsJson['access_token'] = _accessToken;
    parameters.remove(FIREBASE_PHONE_TOKEN);

    firebaseParamsJson['project_id'] = _projectId;
    parameters.remove(FIREBASE_PHONE_PROJECT_ID);

    putValue(parameters, "firebase_phone", firebaseParamsJson);
  }
