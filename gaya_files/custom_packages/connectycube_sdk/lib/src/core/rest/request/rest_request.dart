...
class RestRequest {
  ...
  Future<RestResponse> perform() {
    ...
    switch (_method) {
      ...
      case RequestMethod.PUT:
        response = put(_getUrl(), headers: _headers, body: _getBody(), encoding: utf8);
        break;

      case RequestMethod.PATCH:
        response = patch(_getUrl(), headers: _headers, body: _getBody(), encoding: utf8);
        break;
  ...
  Uri _getUrl() {
    if (_method == RequestMethod.GET || _method == RequestMethod.DELETE) {
      String stringParams = getQueryString(_params);
