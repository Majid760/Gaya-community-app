...
abstract class Query<T> {
  ...
  void putValue(Map<String?, dynamic> parametersMap, String key, dynamic value) {
    if (value != null) {
  ...
  DateTime? getTokenExpirationDateFromResponse(RestResponse response) {
    ...
    if (headers.containsKey(HEADER_TOKEN_EXPIRATION_DATE.toLowerCase())) {
      try {
        ...
        DateTime expirationDate = format.parse(headers[HEADER_TOKEN_EXPIRATION_DATE.toLowerCase()]!, true);
