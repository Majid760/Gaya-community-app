...
Future<void> main() async {
  ...
  group("Tests CREATE dialogs", () {
    ...
    test("testCreatePrivate", () async {
      ...
      String? createdDialogId;

      await createDialog(newDialog).then((createdDialog) async {
        logTime("Success CREATE_PRIVATE_DIALOG");
        createdDialogId = createdDialog.dialogId;
        // assert(createdDialog.description == groupDescription);
        occupantsIds.forEach((userId) {
          assert(createdDialog.occupantsIds!.contains(userId));
        });
        ...
        logTime("Error CREATE_PRIVATE_DIALOG");
        assert(onError == null);
      }).whenComplete(() {
        if (createdDialogId != null) {
          deleteDialog(createdDialogId!, true);
        }
      });
    });
  });
